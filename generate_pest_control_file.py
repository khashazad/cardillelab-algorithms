...
def create_control_file(data, output_filename, observation_count):
    with open(output_filename, "w") as file:
        ...
        file.write(
            f"{line5['RELPARMAX']['value']} {line5['FACPARMAX']['value']}\n"
        )
        ...
        for param in data["parameter_data"]:
            file.write(f"{param['name']} {param['trans']} {param['inctyp']} {param['parval1']} {param['parlbnd']} {param['parubnd']} {param['pargp']} {param['scale']} {param['offset']} {param['dercom']}\n")
...
def create_observations(df):
    ...
    for index, row in df.iterrows():
        ...
        observations.append((observation_name, measurement_value))
        counter += 1
    return observations
    ...
    with open(file_path, 'a') as file:
        ...
        for observation_name, measurement_value in observations:
            if measurement_value != 0:
                file.write(f"{observation_name} {measurement_value} obsgroup\n")

def create_pest_instruction_file(observations):
    with open("eeek.ins", "w") as file:
        file.write("pif *\n")
        for observation_name, measurement_value in observations:
            if measurement_value != 0:
...
if __name__ == "__main__":
    ...
    control_filename = "control_file.pst"
    eeek_output_filename = "./test_run/eeek_output.csv"

    data = read_json(parameters)
    df = read_csv(eeek_output_filename)
    observations = create_observations(df)

    create_control_file(data, control_filename, len(observations))
