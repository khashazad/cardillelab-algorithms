...
def convert_image_collection_to_multi_band_image(image_collection):
    ...
    img_end_result = ee.Image(img_col_list.iterate(iterate_convert_image_collection_to_image, ee.Image(0)))
...
def convert_multi_band_image_to_image_collection(image):
    ...
    band_images = band_names.map(lambda element: 
        ee.Image(ee.Image(image).select([element]).rename('band')).updateMask(ee.Image(ee.Image(image).select([element]).rename('band')).gt(-50))
    )
