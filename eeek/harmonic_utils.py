...
def fit_harmonic_to_collection(harmonic_ic, tracked_band, harmonic_independents):
    ...
    harmonic_trend = harmonic_ic.select(harmonic_independents.add(dependent)).reduce(ee.Reducer.linearRegression(harmonic_independents.length(), 1))
    harmonic_trend_coefficients = harmonic_trend.select('coefficients').arrayProject([0]).arrayFlatten([harmonic_independents])
    myrmsr = harmonic_trend.select('residuals')
    rmse_and_r2 = get_rmse_and_r2(harmonic_trend, harmonic_ic, harmonic_independents, tracked_band)
    the_return = {}
...
def apply_harmonic_to_collection(harmonic_ic, tracked_band, harmonic_independents, harmonic_trend_coefficients):
    harmonic_ic = ee.ImageCollection(harmonic_ic)
    dependent = ee.String(tracked_band)
    harmonic_independents = ee.List(harmonic_independents)
    fitted_harmonic = harmonic_ic.map(lambda image: image.addBands(image.select(harmonic_independents).multiply(harmonic_trend_coefficients).reduce('sum').rename('fitted')))
    return fitted_harmonic

def add_harmonic_bands(value_collection, modality_dictionary):
    vm = modality_dictionary # for convenience of typing
    if vm['bimodal'] or vm['constant'] or vm['linear'] or vm['trimodal'] or vm['unimodal']:
        var_args_time_band = {}
        var_args_time_band['date_property'] = 'frac_doy'
        var_args_time_band['time_band_name'] = "t"
        reduced_collection_with_harmonic = add_time_band_using_property_name_to_ic(value_collection, var_args_time_band)
        if vm['constant']:
            reduced_collection_with_harmonic = add_linear_constant_to_ic(reduced_collection_with_harmonic, 1, "constant")
        if vm['unimodal']:
            var_args_harmonic_bands = {}
            var_args_harmonic_bands['time_band_name'] = var_args_time_band['time_band_name']
            ...
            reduced_collection_with_harmonic = add_cos_and_sin_bands_to_ic(reduced_collection_with_harmonic, var_args_harmonic_bands)
        if vm['bimodal']:
            var_args_harmonic_bands = {}
            var_args_harmonic_bands['time_band_name'] = var_args_time_band['time_band_name']
...
def add_time_band_using_property_name_to_ic(an_ic, var_args_time_band):
    date_property = var_args_time_band['date_property']
    time_band_name = var_args_time_band['time_band_name']
    ...
    the_ic_return = ee.ImageCollection(an_ic).map(add_fractional_year_from_property_name)
...
def add_cos_and_sin_bands_to_ic(an_ic, var_args_harmonic_bands):
    ...
    the_ic_return = ee.ImageCollection(an_ic).map(get_cos_and_sin_for_date_given_angular_freq)
    return the_ic_return

def add_harmonic_independents_using_property_name_to_ic(an_ic, the_frac_doy_property, the_day_range_property, non_standard_period_boolean, non_standard_period_value):
    if non_standard_period_boolean:
        period = non_standard_period_value
    else:
        period = 2
    ...
    def add_fractional_year_from_property_name(an_image):
        ...
        flat_radians = number_for_each_pixel.multiply(angular_frequency).multiply(2).multiply(math.pi)
        ...
        return an_image.addBands(ee.Image.constant(1)).addBands(the_time_band).addBands(the_cos_factor).addBands(the_sin_factor)
    the_ic_return = ee.ImageCollection(an_ic).map(add_fractional_year_from_property_name)
    return the_ic_return
...
def compute_fit_and_get_coefficients(an_ic, band_name_to_fit, harmonic_constant):
    ...
    if harmonic_constant:
        ...
        coefficients = ['β0']
    harmonic_trend = harmonic_imagery.select(independents + [dependent]).reduce(ee.Reducer.linearRegression(len(independents), 1))
    harmonic_trend_coefficients = harmonic_trend.select('coefficients').arrayProject([0]).arrayFlatten([coefficients])
...
def apply_old_coefficients_to_new_ic(a_set_of_harmonic_coefficients, a_prepared_ic, harmonic_constant):
    a_prepared_ic = ee.ImageCollection(a_prepared_ic)
    harmonic_independents = ['constant', 't', 'cos', 'sin']
    harmonic_independents_without_cos = ['constant', 't', 'sin']
    harmonic_independents_without_sin = ['constant', 't', 'cos']
    harmonic_independents_without_cos_and_linear = ['constant', 'sin']
    harmonic_independents_without_linear = ['constant', 'cos', 'sin']
    harmonic_independents_without_sin_and_cos = ['constant', 't']
    harmonic_independents_only_constant = ['constant']
    independents = harmonic_independents_without_linear
    if harmonic_constant:
        independents = harmonic_independents_only_constant
    def multiply_coeffs_by_variables(an_image):
        the_fitted_harmonic_image = an_image.addBands(an_image.select(independents).multiply(a_set_of_harmonic_coefficients).reduce('sum').rename('fitted'))
        return the_fitted_harmonic_image
    ic_fitted = a_prepared_ic.map(multiply_coeffs_by_variables)
    return ic_fitted
